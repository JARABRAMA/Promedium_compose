...
fun GradesScreen(viewModel: GradeViewModel) {
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBarWhitBack(
                title = name, onBack = { viewModel.onBack() }, backTile = stringResource(
                    id = R.string.courses
                )
            ){}
        },
...
fun GradeList(paddingValues: PaddingValues, grades: List<Grade>) {
    Column(
        ...
        Column(Modifier.padding(columnContainerPadding)) {
            grades.forEach {
                GradeItem(it)
            }
        }
...
fun GradeItem(grade: Grade) {
    Card(
        ...
        Column(modifier = Modifier.padding(smallPadding)) {
            TextGrade(grade.name, fontSize = big, fontWeight = FontWeight.Bold)
